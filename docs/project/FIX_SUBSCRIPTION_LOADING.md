...
## StoreKit.framework Embed Setting

**Keep StoreKit.framework set to "Do Not Embed".** System frameworks (StoreKit, Foundation, etc.) are linked from the SDK and must not be embedded in the app bundle. Changing it to "Embed & Sign" can cause App Store rejection or runtime issues.
...
**Fix**:
1. Verify product IDs in `Configuration.storekit` match `SubscriptionService.swift` (`SubscriptionTier` enum)
2. Should be: `com.quackdb.searchconsole.weekly`, `com.quackdb.searchconsole.monthly`
...
- Verify Product IDs are exactly: `com.quackdb.searchconsole.weekly`, `com.quackdb.searchconsole.monthly`
...
6. Fill in:
   ...
   - **Product IDs**: `com.quackdb.searchconsole.weekly`, `com.quackdb.searchconsole.monthly` (**MUST MATCH EXACTLY**)
...
Ensure all the above TestFlight steps are completed with:
   - **Product IDs**: `com.quackdb.searchconsole.weekly`, `com.quackdb.searchconsole.monthly` (EXACT MATCH)
   - **Reference Name**: "Monthly Subscription with 7-day trial"
   - **Duration**: 1 month
   - **Price**: $5.99 (Tier 6)
...
grep -r "com.quackdb.searchconsole" "/Users/tarikzukic/Desktop/Spookers/Search_Console/Search Console/Search Console/"
