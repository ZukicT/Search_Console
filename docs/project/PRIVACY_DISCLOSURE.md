...
This app uses **OAuth 2.0** to authenticate with Google. During onboarding you choose one of two access levels:

- **Read-only** (recommended): [`webmasters.readonly`](https://developers.google.com/webmaster-tools/v1/how-tos/authorizing) — list properties, search analytics, list/view sitemaps, URL Inspection. Does **not** allow [`sites.delete`](https://developers.google.com/webmaster-tools/v1/sites/delete), [`sites.add`](https://developers.google.com/webmaster-tools/v1/sites/add), or sitemap submit/delete in the app.
- **Editing access**: [`webmasters`](https://developers.google.com/webmaster-tools/v1/how-tos/authorizing) — same read access, plus add/remove properties and submit/delete sitemaps. Removing a property from the in-app list calls Google's `sites.delete` API.

Both options also request:

- `email` and `profile` — account identification and display name in the app

Changing access level in Settings requires signing out and signing in again so Google can grant the new scope.
